...
class MC_tools():
    ...
    def pull2saves(self):
        """
        将Onedrive中的最新存档拉取到本地
        :return:
        """
        last_cloud_save_name = self.get_latest_save(self._dst_path)
        last_native_save_name = self.get_latest_save(self._src_path)
        assert last_cloud_save_name.split("_")[2] > last_native_save_name.split("_")[2]
        try:
            shutil.copytree(os.path.join(self._dst_path, last_cloud_save_name), \
